 ...
 */\n"[1 .. $];
	...
	foreach(k, family; functionFamilies) {
		import std.stdio;
		//writeln(">> ", family.fromFilename, " _ ", family.familyOfFunction);
		//string fromFilename;
		//string familyOfFunction;
		if (!family.fromFilename && !family.familyOfFunction) {
			if (family.docs_description.value_string == "") {
				//continue;
			}
			// writeln("SKIPPING ", family);
			// writeln(func.name, " in ", family.fromFilename, "/", family.familyOfFunction);//
			// continue;
		}
		F0: foreach(i, func; family.functions) {

			immutable string[] badSuffixes = [
				"NV", "ARB", "EXT", "OES", "KHR", "AMD", "APPLE", "QCOM", "ATI",
				"SGIX", "MESA", "SUN", "INTEL", "IBM", "ANGLE", "SGIS", "IMG", "GREMEDY",
				"glTbufferMask3DFX",
			];
			foreach(suffix; badSuffixes) {
